...
///     <c>solar/&lt;n&gt;/goal</c> and the co-located <c>lights/&lt;n&gt;/goal</c>). Drives the onboard
///     setpoint <see cref="KeyframeAnimationModule.TimeGoal"/> to <c>fraction × Duration</c>; the sim then
///     integrates the animation itself, so this behaves correctly at any time-warp (the proven unscience
///     <c>red-alert</c> solar-deploy path). Game-thread only.
/// </summary>

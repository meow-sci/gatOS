...
//! Copied from `examples/land-o-matic/src/guidance/ksa_quat.rs` (only the Vec3 import differs).
//!
//! Why port the exact arithmetic instead of using a generic quaternion library? KSA's `Transform` uses
//! the conjugate-left convention `q*·[v,1]·q` (`Brutal.Numerics/double3.cs:761`) with a specific
//! Hamilton product (`Quaternions.cs:7`), paired with `CreateFromRotationMatrix` (`doubleQuat.cs:256`).
//! By reproducing that arithmetic bit-for-bit, the invariant
//! `transform(UnitX, compute_burn_body2cci(p, d)) == d` holds by construction — the same identity KSA
//! relies on when it points a burn — and the [`tests`] verify it.
...
/// the (free, for a hovering skywriter) roll. When thrust is parallel to position (vertical), any
/// orthogonal roll reference is used — the thrust direction is unaffected.
pub fn compute_burn_body2cci(position_dir: Vec3, thrust_dir: Vec3) -> Quat {
    let d = thrust_dir.normalize_or(Vec3::new(0.0, 0.0, 1.0));
    ...
    from_rows(d, s, u) // Body→CCI, x y z w, matching KSA's CreateFromRotationMatrix
}
...
mod tests {
    ...
    /// THE invariant: the burn quaternion aims body +X along the thrust direction, for any
    /// position/thrust pair — including the vertical (degenerate-roll) case, which for a hovering
    /// skywriter is the *common* case.
    #[test]

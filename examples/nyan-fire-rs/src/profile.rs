...
//! templates it runs on. A profile is the ordered stripe palette plus every [`crate::app::Settings`]
//! knob (frame rate, fade steps, the stripe clock, the two staggers, the window width, the whole
//! brightness block, and the batch-dispatch flag), serialized to a small, hand-editable YAML file.
//! Templates are deliberately **not** saved: a profile is the look of the burn, and you re-pick which
//! plumes it plays on each run (`--profile <name>` loads the file, then you arm templates as usual).
//!
//! Storage is a flat directory of `<name>.yaml` files. The directory is `$NYAN_PROFILE_DIR` if set,
...
use crate::app::{Settings, BRIGHT_MAX};
use crate::color::{self, Rgb};

/// A reusable show: the ordered palette + every display knob. The selected templates are intentionally
/// excluded (see the module docs) — a profile is re-applied to whatever plumes you arm this run.
...
const MAX_SLOT_STEP: i64 = 8;

impl Profile {
    /// Serializes to the flat YAML form documented above.
    pub fn to_yaml(&self) -> String {
        let s = &self.settings;
        let mut out = String::from("# nyan-fire-rs profile\n");
        out.push_str(&format!("hz: {}\n", fmt_f(s.hz)));
        out.push_str(&format!("steps: {}\n", s.steps));
        out.push_str(&format!("color_ms: {}\n", s.color_ms));
        out.push_str(&format!("slot_stagger_ms: {}\n", fmt_f(s.slot_stagger_ms)));
        out.push_str(&format!("slot_step: {}\n", s.slot_step));
        out.push_str(&format!("tpl_stagger_ms: {}\n", fmt_f(s.tpl_stagger_ms)));
    ...
    pub fn from_yaml(text: &str) -> Result<Profile, String> {
        ...
        for raw in text.lines() {
            ...
            match key {
                ...
                "slot_stagger_ms" => {
                    if let Ok(v) = value.parse::<f64>() {
                        settings.slot_stagger_ms = v.clamp(0.0, MAX_STAGGER);
                    }
                }
                "slot_step" => {
                    if let Ok(v) = value.parse::<f64>() {
                        settings.slot_step = (v.round() as i64).clamp(0, MAX_SLOT_STEP) as u32;
                    }
                }
                "tpl_stagger_ms" => {
                    if let Ok(v) = value.parse::<f64>() {
                        settings.tpl_stagger_ms = v.clamp(0.0, MAX_STAGGER);
                    }
                }
                "bright_min" => {
                    if let Ok(v) = value.parse::<f64>() {
                        settings.bright_min = v.clamp(0.0, BRIGHT_MAX);
                    }
                }
                "bright_max" => {
                    if let Ok(v) = value.parse::<f64>() {
                        settings.bright_max = v.clamp(0.0, BRIGHT_MAX);
                    }
                }
                "bright_ms" => {
                    if let Ok(v) = value.parse::<f64>() {
                ...
                // Anything that isn't an affirmative reads as false; a missing key keeps the default.
                "batch" => {
                    settings.batch = matches!(value.to_ascii_lowercase().as_str(), "true" | "1" | "yes")
                }
...
/// The directory profiles live in (see module docs). Honors `$NYAN_PROFILE_DIR`, else a per-user
/// `~/.nyan-fire/profiles`, else a relative `nyan-profiles`.
pub fn profiles_dir() -> PathBuf {
    if let Ok(d) = std::env::var("NYAN_PROFILE_DIR") {
        if !d.is_empty() {
    ...
    if let Ok(home) = std::env::var("HOME") {
        if !home.is_empty() {
            return PathBuf::from(home).join(".nyan-fire").join("profiles");
        }
    }
    if let Ok(up) = std::env::var("USERPROFILE") {
        if !up.is_empty() {
            return PathBuf::from(up).join(".nyan-fire").join("profiles");
        }
    }
    PathBuf::from("nyan-profiles")
}
...
mod tests {
    ...
    fn profile() -> Profile {
        Profile {
            settings: Settings {
                ...
                steps: 1,
                color_ms: 220,
                slot_stagger_ms: 80.0,
                slot_step: 2,
                tpl_stagger_ms: 200.0,
                bright_min: 20.0,
                bright_max: 60.5,
                bright_ms: 400,
                bright_steps: 6,
    ...
    fn out_of_range_values_are_clamped() {
        ...
        assert_eq!(p.settings.bright_max, BRIGHT_MAX); // the leaf's real 0..200 ceiling
        assert_eq!(p.settings.slot_step, MAX_SLOT_STEP as u32);
    }

    #[test]
    fn batch_flag_round_trips() {
        // Saved verbatim, and every non-affirmative spelling reads as off.
        let mut p = profile();
        p.settings.batch = false;
        assert!(p.to_yaml().contains("batch: false"));
        assert!(!Profile::from_yaml(&p.to_yaml()).unwrap().settings.batch);
        assert!(Profile::from_yaml("batch: yes\n").unwrap().settings.batch);
        assert!(Profile::from_yaml("batch: 1\n").unwrap().settings.batch);
        assert!(!Profile::from_yaml("batch: nope\n").unwrap().settings.batch);
        // A missing key keeps the default (on).
        assert!(Profile::from_yaml("hz: 30\n").unwrap().settings.batch);
    }
    ...
    fn resolve_path_distinguishes_names_from_paths() {
        std::env::set_var("NYAN_PROFILE_DIR", "/tmp/nf");
        assert_eq!(resolve_path("showtime"), PathBuf::from("/tmp/nf/showtime.yaml"));
        assert_eq!(resolve_path("my show!"), PathBuf::from("/tmp/nf/my_show_.yaml"));
        // A path-like argument is used verbatim.
        assert_eq!(resolve_path("./x.yaml"), PathBuf::from("./x.yaml"));
        std::env::remove_var("NYAN_PROFILE_DIR");
    }

    #[test]
    fn save_then_load_on_disk() {
        // Use a path-like name (handled verbatim by `resolve_path`) so this doesn't depend on the
        // process-global `$NYAN_PROFILE_DIR` (which parallel tests would race on).
        let dir = std::env::temp_dir().join(format!("nyan_prof_{}", std::process::id()));

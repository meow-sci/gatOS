...
//! exactly the format the `/sim` `debug/engineplume/templates/<id>/emission/color<n>` control file
//! wants (`"r g b"`, each 0..1). The user, though, enters colors the friendly way — 0..255 channels
//! or an HTML hex string — or picks from the bundled [`crate::xkcd`] survey palette, so this module
//! is the bridge: parse those human formats in, interpolate between them, and format back out to the
//! `/sim` wire form.
//!
//! (Forked from `examples/dancy-party-rs/src/color.rs`; the only code difference is that `Rgb::scaled`
//! is gone — plume brightness is its own `emission/brightness` leaf on a 0..200 scale, not a colour
//! multiplier, so a palette colour is written verbatim.)
...
impl Rgb {
    ...
    /// precision for a gradient stop (the game stores it as an f32 anyway), and short enough to keep
    /// the 9p write tiny.

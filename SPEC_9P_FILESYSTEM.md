...
| `scale` | **St** | scalar | Uniform model scale factor; read = current (best-effort; `1` = unscaled/unknown), **write any finite value > 0** to rescale the whole vessel model (action `vessel.scale`, Frame, one-shot). Default `1.0`; no upper clamp; `0`/negative/non-finite → `EINVAL`. Exempt from the active-vessel authority gate — works on **any** vessel by id. Top-level part scales are saved by KSA; staging/undocking need not reset them. Subpart transforms and the EVA avatar scale have separate lifetimes. First per-vessel control intentionally placed here rather than under `/sim/debug/`. ⚠ **This is a visual/transform-only scale, and as of KSA `2026.8.22.5348` it deliberately no longer means what the in-game gizmo means.** The live write changes model transforms without recomputing colliders, mass, volume or performance. **Saving and reloading can make the scale physical:** KSA stores top-level `Part.Scale` and, since `2026.9.10.5438` (rev 5434), calls `RefreshScale()` for every loaded non-root part as well as the root. That can rescale colliders, mass, tanks and nozzle geometry. Restore the intended top-level scale before saving if the change should stay cosmetic. Rev 5329's `IRescale` made the **game's own editor scaling physical** (colliders, tank volume, inert mass, nozzle areas, decoupler separation force), clamped to **0.5×–2×** and quantized to 0.25 m diameter steps; `/sim` scale does none of that and still admits any finite value > 0. |
...
| `debug/fx/profiles` | S | csv | — | — | The profile vocabulary, comma-separated: `party,sparkle,danger,death`. |
| `debug/fx/spawn` | **St** | `<vessel> <profile> [scale <s>] [offset <x> <y> <z>]` | `debug.fx_spawn` | Frame | Burst one particle effect on a vessel, anchored — by default — at an EVA kitten's **face** (assembly `(0.25, 0, -0.70)`; non-kitten vessels default to their assembly origin; `offset` overrides, metres, assembly frame). `scale` (> 0, default 1) multiplies size/radius/velocity. Keyword groups are order-independent, each at most once. One-shot `Burst` emitters from the game's own particle pool: they self-retire when every particle decays. Since KSA `2026.9.10.5438`, all four profiles use density-based buoyancy with zero authored drag: at atmospheric density 1.225 kg/m³, `party` falls gently, `sparkle` is neutral, and `danger`/`death` rise gently. Motion varies with local atmosphere; below 100 Pa every profile receives full local gravity. Errnos: EINVAL (grammar), ENOENT (vessel), EBUSY (gatOS cap or the game's pool exhausted), EOPNOTSUPP (graphics `Particles` setting off). Read = empty. |
| `debug/fx/count` | S | int | — | — | Live gatOS-held emitters (decays to zero on its own as bursts finish). |
| `debug/fx/clear` | T | `1` | `debug.fx_clear` | Frame | Stop every gatOS effect now (graceful: emission stops, existing particles live out their lifespan). Vessel-agnostic. |
...
(settings-changed → volumetric-exhaust limits recompute), so a burning engine repaints within the

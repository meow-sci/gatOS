...
| `debug/fx/spawn` | **St** | `<vessel> <profile> [scale <s>] [offset <x> <y> <z>]` | `debug.fx_spawn` | Frame | Burst one particle effect on a vessel, anchored — by default — at an EVA kitten's **face** (assembly `(0.25, 0, -0.85)`; non-kitten vessels default to their assembly origin; `offset` overrides, metres, assembly frame). `scale` (> 0, default 1) multiplies size/radius/velocity. Keyword groups are order-independent, each at most once. One-shot `Burst` emitters from the game's own particle pool: they self-retire when every particle decays. Errnos: EINVAL (grammar), ENOENT (vessel), EBUSY (gatOS cap or the game's pool exhausted), EOPNOTSUPP (graphics `Particles` setting off). Read = empty. |

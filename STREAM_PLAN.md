...
re-decode fix + real-frame regression tests. **2026-07-02 (perf plan P6): the purrTTY native was
rebuilt from ghostty main + the `purrtty/vt-video-fixes` patches — the `o=z` corruption is FIXED
and `display_encoding` defaults to `rgba-zlib` again** (raw stays selectable). In-game live-video
validation (S6/S9) pending; S8 deferred.** This document is
...
1. **The pinned libghostty-vt native memory-corrupts on kitty `o=z` (zlib) payloads of compressible
   ...
   **pixel-exact** vs the ground-truth PNGs; PNG `f=100` is silently ignored (unsupported) by this
   native build. Quarantine (historical): gatOS defaulted `display_encoding=rgba` until the native was fixed
   (2026-07-02, perf plan P6 — see below); purrtty gotcha 34 tracked the native bug and the
   `[Explicit]` crash repro gated every pin bump.
   *Attribution (2026-07-02, verified against the ghostty repo + a standalone zig repro):* the fault
   ...
   (ziglang/zig #25032/#25035, milestone "urgent") are still open. **RESOLVED 2026-07-02 (perf plan
   P6): the native was rebuilt from ghostty main `c22df09da` + the `purrtty/vt-video-fixes` branch,
   whose `decompressZlib` patch routes around the std bug (windowless direct mode into a
   `Writer.Allocating`); the un-quarantined `ZlibRealFrame_DecodesToGroundTruth` decodes the real
   `o=z` frame pixel-exact and the default is `rgba-zlib` again.** Details: purrtty gotcha 34.

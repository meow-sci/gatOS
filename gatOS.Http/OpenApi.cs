    ...
    internal const string Document =
        """
        {
          ...
          "paths": {
            ...
            "/paint/texture/files": { "get": { "summary": "Uploaded clutter-texture images (name, bytes, kind, version, ready); /v1/paint/textures/... is an alias. These images are also what sticker decals draw \u2014 /sim/paint/stickers has no upload route of its own, and every other sticker leaf is an ordinary /v1/fs mirror", "responses": { "200": { "description": "image list" }, "404": { "description": "custom clutter textures disabled" } } } },
            "/paint/texture/file/{name}": {
              "put": { "summary": "Binary image upload (raw body; ?offset=N appends by position for chunking, ?complete=0 keeps the upload open — default commits). POST is an alias. Unlike /audio/file, a Content-Length over the 1 MiB request cap is refused up front with 413 EFBIG instead of committing an empty body, so images over 1 MiB must be chunked.", "parameters": [{ "name": "name", "in": "path", "required": true, "schema": { "type": "string" } }, { "name": "offset", "in": "query", "schema": { "type": "integer", "default": 0 } }, { "name": "complete", "in": "query", "schema": { "type": "integer", "default": 1 } }], "requestBody": { "required": true, "content": { "application/octet-stream": { "schema": { "type": "string", "format": "binary" } } } }, "responses": { "200": { "description": "{outcome:ok,name,bytes,ready}" }, "400": { "description": "EINVAL (bad name / non-sequential offset)" }, "413": { "description": "EFBIG (body over the 1 MiB request cap, or the per-image cap)" }, "507": { "description": "ENOSPC (store/count cap)" } } },
              "delete": { "summary": "Evict an uploaded image (any binding using it is unbound first; sticker decals using it go dormant, not deleted)", "parameters": [{ "name": "name", "in": "path", "required": true, "schema": { "type": "string" } }], "responses": { "200": { "description": "{outcome:ok}" }, "404": { "description": "ENOENT" } } }
            },
            "/command": {
              "post": {

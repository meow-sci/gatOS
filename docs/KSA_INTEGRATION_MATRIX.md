...
**Current upgrade: 2026-09-14 against `2026.9.10.5438`, from audited `2026.9.7.5402`.**
Full gapless source/Content review (35 revisions, 5403–5437) identified particle gravity/density,
exhaust propagation and enum compile breaks, plus paint/IVA overload ambiguity and sticker
color-only resolve breaks. Updated anchors carry this audit date and build; untouched anchors
retain their historical per-site verification dates. Full evidence, binary survey and final build/
test results: [5438 pass](../scope/ksa-assets-and-versions.md#5438-pass).
Live game checks remain pending; no static result is a visual or flight validation.

| Changed anchor / site | 5438 binding and behavior | Risk |
|---|---|---|
| `PartPaintPatches.Resolve`, `PaintManager.TryGetPartBits` | Explicit private `PartModel` and `PartModelDynamic.AddInstance(PerInstanceData,PerInstanceDent,IViewport,int)`; both dented and ordinary public paths pass through once; state bits 11–31 remain available | H |
| `IvaForceRender.SetEnabled` | Same private common `PartModel.AddInstance`; paired `InstanceList`/`DentInstanceList` append; actual viewport gate | H |
| `StickerRenderPatches.Apply` | Exact `RenderTarget.ResolveAttachments(CommandBuffer,bool)`; skip color-only false and retain main-target, main-viewport and editor filters | H |
| `PlumeActuator.Propagate` | `OnSettingsChanged` + `RocketNozzle.RecomputeVolumetricExhaustLimits`; delete obsolete `FxReflect.PlumeModifierArgs` | H |
| `FaceFxManager.Spawn` | `ParticleEmitterReference.Density`/`Drag`; native atmosphere-dependent buoyancy, standard-density calibrated profiles, zero drag | M |
| Thug-life/sticker index draws | `VkIndexType.UInt16 = 0`; existing ushort buffers | H |
| `ScaleActuator.Apply` | Live transform-only write; full-part scale saved; rev 5434 refreshes physics for every loaded non-root part too | H |

**Prior verification:** **2026-09-02 against `2026.9.7.5402`** (clean `-t:Rebuild` of the whole solution against
...
key on the vessel **id** (they survive scene rebuilds) and are pruned when the vessel despawns (`VesselForceRender.Prune`, riding the sampler's
...
method (`KSA.Rendering.RenderTarget.ResolveAttachments(CommandBuffer,bool)`, not
...
|---|---|---|---|
| `StickerRenderPatches.Apply` | exact dynamic Harmony **postfix on `KSA.Rendering.RenderTarget.ResolveAttachments(CommandBuffer,bool)`** (`KSA.Rendering/RenderTarget.cs:315`; full main resolve `KSA/Program.cs:4765`) | Skip `inResolveDepth=false` (early color-only resolve at `Program.cs:4737`). Draw once after full resolve even at MSAA 1×. Require active manager, main target, main viewport and `!Program.EditorFlag`; dynamic install/remove lifecycle unchanged. | **H** |

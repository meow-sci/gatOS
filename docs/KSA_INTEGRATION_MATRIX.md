...
| `FrameCapture.MaybeRecord` | `Program.GetRenderer()`, `Program.MainViewport.OffscreenTarget.ColorImage`/`.Extent`, `Renderer.Allocator`/`.MaxFramesInFlight`, `Program.ResourceFrameIndex`, `Allocator.CreateImage`/`CreateBuffer`, `CommandBuffer.PipelineBarrier`/`BlitImage`/`CopyImageToBuffer`, `BufferEx.Map` | M | Records the downscale-blit + copy-to-host **into the engine's own frame command buffer** (no out-of-band submit, no `WaitIdle` — that crashed the game). Mirrors `KSA/PlanetMapExporter.cs`. Offscreen `ColorImage` is `SampledReadVfc = ShaderReadOnlyOptimal` at `RenderGame` end (`Program.cs:4125`) and is restored. Deferred readback via the `ResourceFrameIndex` frames-in-flight slot ring (the slot's prior copy is complete when reused — no fence wait). Engine types reached by inference + interface constraints. |

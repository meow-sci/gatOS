...
| 2 | The quad is **correctly oriented** and **depth-tested** — it is occluded by geometry in front of it (NOT painted on top of everything) | ☐ | verifies the `Program.OffscreenTarget` dynamic-rendering pipeline stamp + **reverse-Z** depth |
...
| 6 | Force MSAA **4×** and **8×** → no depth/edge artifacts on the quad | ☐ | `SetupGraphicsPipeline` takes the sample count from the live target |
...
| 5 | **plumetrail, live effect:** `echo 200000 > /sim/debug/plumetrail/render/max_distance` and `echo 0.2 > …/render/erosion_max_depth` change the trails with no apply call; values read back. (`render/trail_color` was **retired at KSA 2026.9.7.5402** — the global tint left the renderer; `ls render/` must not list it) | ☐ | public fields re-read each frame |
...
| 10 | **terrain, per body:** `echo 0.35 > /sim/debug/terrain/bodies/<body>/tessellation/factor`, `echo 9000 > …/max_height`, `echo 45 > …/slope_roughness_deg` each visibly change the surface **with no flicker or strobing over several seconds** | ☐ | the frames-in-flight UBO mirror copy — flicker here means the mirror loop is wrong. **5348 (C3):** that mirror is now **field-wise**, not a whole-struct copy — revs 5319–5325 gave `MeshUbo` four per-frame split-double terrain anchors (`DirAnchorHi/Lo`, `DirAnchorUvHi/Lo`) that `GenerateMeshData` rewrites from the live camera every frame and the old copy was stamping over. **D9:** `tessellation/range_m`'s default moved **220 → 50** and the shader's displacement falloff moved to `range*0.75 … range*0.975`, so the documented example values read differently now |
...
| 4 | `echo 1 > ctl/stage` still activates the next sequence and `ctl/stage`-driven decouples behave (staging window in-game is now "Resource Groups") | ☐ | `SequenceList.ActivateNextSequence` intact; sequences double-buffered (4880) + batched spent-sequence removal (4873). **Semantics moved at 5348/rev 5329 (D1)** — see the 5348 section's item 10 |
...
| 5 | `echo 1 > ctl/stage` on a controllable vessel still activates the next sequence (the in-flight Sequences window is redesigned + re-orderable in flight) | ☐ | `SequenceList.cs` +1137-line UI rework; `ActivateNextSequence` byte-compatible. **Superseded at 5348/rev 5329 (D1):** the signature still holds but the body now calls `Parts[n].ActivateSubtreeInStage(vehicle, sequence.Number)`, which walks the **subtree** and activates only `ISequenced` modules whose own `Sequence` matches — so staging no longer activates RCS, and a part with modules in two sequences needs two presses. See the 5348 section's item 10 |
...
| 5 | In an orbit whose period greatly exceeds the target's, `encounters/` no longer floods with entries | ☐ | revs 5106/5110 — row count/content change only, struct unchanged. **Corrected in the 5348 pass:** this item used to add "and the listed approaches reflect the **final** trajectory including planned burns". That was never true of `Patch.Encounters` — gatOS reads `PatchedConic._encounters` (SOI encounters), while the game's target gauge reads `_closestApproaches` (rev 5266 rewired the gauge onto `FlightPlan.TryFindNextClosestApproach(target, now)`, an earliest-in-time scan of a **different** list, replacing a global-minimum scan over `FindFinalFlightPlan().Patches`). `Vehicle.FindFinalFlightPlan()` — the API that would have described a final trajectory — was **deleted in 5348**, and gatOS references it nowhere. `Vehicle.Patch.Encounters` and `Encounter.{Body,GameTime,ClosestDistance}` are themselves unchanged |
...
## KSA 2026.8.22.5348 upgrade — live re-check items — **NOT YET RUN**

The 2026.8.19.5261 → 2026.8.22.5348 playbook pass (2026-08-23, `fromRevision` 5261 → `toRevision` 5348,
85 commits, revs 5262–5348) found **zero compile breaks** — the first pass in the project's history
with none (5261 had ten, 5168 four). Prereq: the mod built + deployed against the 5348 DLLs; build and
full suite were green (**0 warnings, 1646 passed / 12 skipped / 0 failed**). Six code changes landed
anyway, one of them a **brand-new KSA coupling** (a Harmony prefix on `GameSettings.UiPixelCulling()`
so `/sim/display` captures complete frames), plus nine silent semantic drifts documented but not fixed.
This was a **static + binary-metadata** review — the binary surface diff proves every reflection
accessor and Harmony target is *present with a compatible shape* in the shipping 5348 assemblies, but
render correctness and in-flight behaviour genuinely need a live pass. The static findings are in
[`../scope/ksa-assets-and-versions.md#5348-pass`](../scope/ksa-assets-and-versions.md#5348-pass). All
prior items above remain valid and can run on 5348. Residual items static review cannot settle:

| # | Check | Result | Notes |
|---|---|---|---|
| 1 | **`/sim/display` captures complete frames with opaque game UI open** — open several windows and the HUD, stream, and confirm there are no black rectangles where the windows/HUD sit; then turn the stream off and confirm the culling optimization comes back (the game's own UI-occluded pixels are skipped again) | ☐ | **the C1 fix and the highest-risk item of this pass.** rev 5283 added `UiCoverageMaskSystem`: `RecordDepthStamp` stamps the reverse-Z near plane into the opaque pre-pass wherever fully-opaque ImGui UI covers the screen, and `FrameCapture` reads the offscreen colour **before** the UI composite — invisible locally, only a remote reader sees it. The prefix on `GameSettings.UiPixelCulling()` returns `false` while capturing; it is best-effort, so a silent miss costs fidelity, not the stream |
| 2 | `cat /sim/status/accessors` is clean (no degraded latches) after a flight exercising throttle, translate/rotate, staging and an FC setpoint | ☐ | standing post-update check. Reflection resolution and Harmony **install** cannot be build-checked; the binary diff confirmed all ~15 accessors and every Harmony target present with compatible shapes in the real 5348 DLLs, but only a live load proves they resolve. Watch the new `gatos.display` prefix (C1) alongside the existing set |
| 3 | `cat /sim/paint/textures/clutter` now returns **rows**, and `bind` on one of those content-relative ids actually re-points the texture in flight | ☐ | **C4** — the catalog was keyed on `GetRealId()` and published **empty** on both 5261 and 5348 (no clutter texture element carries an `Id=` attribute), so every `bind` answered `ENOENT` and the feature was inert. Re-keyed on `TextureReference.LocalPath`; ids are now paths like `Textures/Planets/Earth/GroundClutter/Grass_Diffuse.ktx2`. The new `alpha` slot (**C5**, rev 5348's `PbrMaterialReference.AlphaMap`) should be absent — no stock material authors one |
| 4 | **Welds track rigidly** through translation, rotation and time-warp, and **survive a bubble split/rejoin** | ☐ | **joint-highest risk with item 1.** revs 5331/5339 moved `TrimBubbles()`/`IntakeOrphans()`/`MergeBubbles()`/`SplitBubbles()` into `VehicleUpdateTask.Run()` on the worker, with a **2.0× split hysteresis** and a cached pair-clearance dictionary; `Universe.{MergeVehicleTasks,TrimPhysicsBubbles,AddVehiclesToTasks}` and `Universe._physicsBubbles` are deleted. A per-tick weld teleport orphans and re-intakes the vessel **every tick** against that new machinery. Statically safe: `WeldManager.Update` already calls `JobSystems.VehicleSolver.Wait()` first and `Universe.GetJobSimStep` has zero diff |
| 5 | `debug/teleport` and `debug/impulse` still actuate correctly **from the Solver lane**, with no log spam and no physics blowup | ☐ | **C2** — both moved Frame → Solver because they reach `PhysicsBubble.RemoveVehicle` (`_vehicleStates.Remove`, `TopologyVersion++`, `ConstraintSim.RemoveVehicle`) and bubble structure is now mutated on the solver thread (revs 5331/5339). The job is in flight from the tail of `Program.PrepareFrame` (`:2047`) to the next frame's `Wait()` (`:2010`) — the whole GUI phase where the Frame lane drains; the gatOS solver prefix runs in the one provably safe window. `Vehicle.Teleport`'s signature and null-semantics are unchanged. Also watch `ctl/stage`: `StagingActuator.Stage` still calls `UpdateAfterPartTreeModification()` from the Frame lane — pre-existing, not widened, recorded not fixed |
| 6 | `/sim/debug/terrain` `min_height`/`max_height`/`biomes/blend_strength` writes apply **without a one-frame terrain twitch**, and the tessellation example values still read sensibly at the new 50 m default | ☐ | **C3** — revs 5319–5325 (terrain precision rework) gave `MeshUbo` four per-frame split-double anchors (`DirAnchorHi/Lo`, `DirAnchorUvHi/Lo`) written from the live camera every frame; gatOS's whole-struct frame-0 mirror was stamping frame 0's anchor over every other frame-in-flight (self-healing but real). The mirror is now field-wise. **D9:** `TessellationRangeMeters`' default went 220 → 50 |
| 7 | `thug_life` quad still draws correctly positioned, sized, depth-tested and occluded — including in a **supersampled screenshot**, and with crew portraits both **enabled and disabled** | ☐ | **D7** — revs 5276/5295 mean the crew-portrait viewports are **no longer always `Visible`** (`GameSettings.ShowCrewPortraitCameras()` gate + `Visible = k < _visibleCount`), and `RenderViewport` is gated on `Visible`, so with portraits off `RenderMainPass` and the gatOS postfix never run for them. `CurrentPassBit()` still classifies correctly (`_crewPortraitViewportStart = 4` unchanged); the `Cameras & Crew` bit just goes unused. `RenderMainPass` is also now wrapped in a `TagRegion`, so gatOS's draws are attributed outside that GPU tag — profiler attribution only |
| 8 | **Stickers:** a live sticker still draws — check one at a **launch pad**, where terrain is now decal-flattened and new static-object geometry sits, and one near a **cube-face seam** | ☐ | revs 5335–5337 added KSA's own "Decal" (`DecalModifierReference` deforms terrain *height* via a `HeightMap`; it does **not** paint colour, so it does not overlap gatOS stickers), five launch-site flatteners on Earth, and `Landmark` entries carrying `StaticObject="CoreLaunchPadA_Prefab_LaunchPadA"`. **D8:** `SampleCubeFacePointR`'s out-of-range branch replaced a 4-tap bilinear seam fetch with `UnfoldCubeFaceUv` + one clamped nearest tap, shifting `StickerAnchors`/`StickerPicker` terrain anchoring sub-metre near cube-face boundaries. The seam itself is statically verified unchanged (`KSA.Rendering/RenderTarget.cs` untouched) |
| 9 | **EVA paint:** enumerate the `mmu` / `mmu.N` slots and confirm the **count and meaning** after the MMU mesh and material reorder | ☐ | **D5**, rev 5268 era: `CharacterAssets.xml`'s `CharacterMMUAttachment` moved to the **skinned** `SK_KSA_MMU.glb` (`MmuMesh` retyped `StaticMeshRenderable` → `AnimatedRenderable`) and its two `<Materials>` blocks were reordered — `KSA_MMU_Color` is now index **0** (was 1), `KSA_MMU_Texts` index **1** (was 0). A saved rule targeting `mmu` now repaints the **body** instead of the label decals, and because the `.glb` is not in the repo the `MaterialIndices` array **length** could also differ |
| 10 | `ctl/stage` on a vessel with RCS — confirm `rcs/<n>/active` **no longer flips as a side effect**, and that a part with modules in different sequences needs **two presses** | ☐ | **D1**, rev 5329: `ActivateNextSequence`'s body went `Parts[n].ActivateInStage(vehicle)` → `Parts[n].ActivateSubtreeInStage(vehicle, sequence.Number)`, which walks `GetSubtreeSequencedModules()` and activates only modules whose own `Sequence` matches. `ISequenced` implementors are exactly `EngineController` and `Decoupler`; `ThrusterController` is `IActivate` but **not** `ISequenced`. Third sub-change to check: it is now a **subtree** walk, so engines/decouplers on sub-parts are staged where they were previously skipped |
| 11 | `ctl/engines/<n>/min_throttle` on a **multi-engine stack** — confirm the inverted aggregation: the effective floor is now the **most** restrictive engine, not the least | ☐ | **D2**, rev 5317 era: `FlightComputer.ComputeActiveEnginePerformance`'s fold seeded `1f` → `0f` and `MathF.Min(num, MinimumThrottle)` → `MathF.Max(…)`. `EngineController.MinimumThrottle` itself is unchanged and gatOS's write still lands; only `ActiveEnginePerformance.MinThrottle` (the clamp floor in `SolveBurnThrottle` and the manual path) moved, and the empty-set default flipped 1.0 → 0.0 |
| 12 | `navball/deltav` and `navball/twr` read **correct** values on a vehicle with a part in **sequence 0** | ☐ | **D3**, rev 5318: `Vehicle.UpdateNavballData`/`NavBallData` unchanged, but `SequencePerformanceList` now iterates `part.GetSubtreeSequencedModules()` and matches each module's own `Sequence` (as does decoupler jettison-mass attribution). This is the "a part assigned to sequence 0 silently zeroed the vehicle's Δv and TWR" fix — the values were **wrong before**, so these reads will differ, correctly, on affected vehicles |
| 13 | A burn planned via `ctl/burn` completes in a **plausible time** with the retuned TVC gains | ☐ | **D4**, rev 5317: `BurnTarget` gained `float? Throttle` (auto-burn throttle now latched on the target, seeded from `PlannedBurnThrottle`) and `bool LastIgnitionDenied`; `MAX_TRANSIENT_BURN_FRACTION` 0.3f → 0.5f; `TVC_SETTLE_TOLERANCE 0.02` → `TVC_SETTLE_POINTING_TOLERANCE 0.0017453` (0.1°); the TVC gain matrix went `(10000, 1000)` → `(50, 100)` alongside a row-vs-column multiplication **bug fix**; `HasAnyPropellant` was replaced by a two-consecutive-denials latch (the "always kicked out of auto burn on save load" fix). Burn duration, throttle profile and the abandon point all differ from the 5261 baseline for the same commanded Δv |
| 14 | `/sim/debug/fx` face bursts land on the kitten's **face** at the new 0.70 offset | ☐ | **C6**, rev 5270 lowered `CrewPortraitPanel.FACE_HEIGHT_OFFSET_EVA` 0.85 → 0.7 ("so the portrait frames the kitten's face instead of the top of the helmet"); gatOS mirrors that constant, so bursts were spawning ~0.15 m too high. Note for later: with a non-`Default` bone target the game's face point is now `kitten.Renderable.GetBoneAsmbOffset(boneIdx)` — there is no single constant to mirror, and the new public `KittenRenderable.{TryFindBoneIndex,GetBoneAsmbOffset}` could track it exactly |
| 15 | **Vulkan 1.4:** confirm the mod still loads on any borderline GPU/driver | ☐ | rev 5315 moved the engine 1.3 → 1.4. gatOS declares no API version, no extensions and no features and reuses `Program.GetRenderer().Device`, and `ShaderModuleUtils` maps 1.4 to SPIR-V `_1_6` — **the SPIR-V target is unchanged**, so the runtime-compiled sticker GLSL and the `UnlitMesh*` shaders produce the same SPIR-V. Environment note only: the mod now inherits a Vulkan 1.4 device requirement. Worth pairing with a check that no **stale SPIR-V cache** survives the rev-5301 lighting-UBO reshape, which grew the `UboLightingData` stride |

## KSA 2026.9.7.5402 upgrade — live re-check items — **NOT YET RUN**

The 2026.8.22.5348 → 2026.9.7.5402 playbook pass (2026-09-02) ran against a **gapped changelog**
(CURRENT's `version.json` logs only rev 5401; revs 5349–5399 have no messages), so the full decomp +
Content diff was the discovery mechanism. **Three compile breaks, all fixed:** the `Viewport` class
rework (nine files retyped; the camera director's `Mode`/`FixedController` writes now go through a
**reflection seam**, `Game/Ksa/Camera/ViewportSeam.cs`), `VolumetricTrailRenderer.DebugTrailColor`
removed (`/sim/debug/plumetrail/render/trail_color` **retired**), `Cursor.InputRay` removed (sticker
cursor aim now `Cursor.GetEgoRay`). Prereq: the mod built + deployed against the 5402 DLLs; build and
full suite were green (**0 warnings, 1646 passed / 12 skipped / 0 failed**). The binary surface diff
proved every member reference, reflection string and Harmony target resolves in the shipping 5402
assemblies; what it cannot prove is that the new reflection seam *works* in a live viewport, that the
render passes still draw where they should under the new viewport types, and that the new engine
behaviours (debris, parachutes) look the way the docs now say. Static findings:
[`../scope/ksa-assets-and-versions.md#5402-pass`](../scope/ksa-assets-and-versions.md#5402-pass). All
prior items above remain valid and can run on 5402. Residual items static review cannot settle:

| # | Check | Result | Notes |
|---|---|---|---|
| 1 | **Camera take through the reflection seam:** `echo 1 > /sim/camera/enabled` answers OK (not `EOPNOTSUPP`), the view parks in fixed mode with **no** "Fixed Camera" alert, a pose write moves the camera, and `echo 0` restores mode/follow/FOV exactly; `cat /sim/status/accessors` stays clean and the log carries no `ViewportSeam` warning | ☐ | **C1** — `GameViewport.FixedController` and `ViewportBase.Mode` are protected-set properties now; `ViewportSeam` invokes their compiler-generated setters by reflection. A miss degrades to `EOPNOTSUPP` (controller) or to `SetCameraMode` (mode) — both logged once |
| 2 | **Held RCS input survives a camera take/release:** latch `echo 1 > ctl/translate/fwd`-style flags, then `camera/enabled 1` and `0`; the flags are still set afterwards (i.e. the seam's direct `Mode` write, not the `SetCameraMode` fallback, was used) | ☐ | the reason the seam exists — `SetCameraMode` calls `ClearHeldPlayerInput()`; also the `Program.ControlledVehicle` setter now clears held input on a vessel switch |
| 3 | **Camera clamp near terrain:** place an owned camera `geo` 0 m above terrain / slightly below it — the read-back reports ~0.5 m above the surface, no jitter, no fall-through | ☐ | `Camera.ClampCamera` is now camera-local + terrain-aware (`TryGetSurfaceClampPositionEcl(0.5)`) instead of keyed on the frame viewport's altitude |
| 4 | **Sticker cursor aim:** `paint.sticker_place` with `cursor` lands the decal under the mouse pointer (main window, and with a secondary camera window open the aim still uses the **main** viewport's camera) | ☐ | **C3** — `Cursor.GetEgoRay(Program.MainViewport)` computes the ray live from the desktop cursor minus `viewport.Position`; the previous `InputRay` was a per-frame cache |
| 5 | **thug_life across the new viewport types:** with `cameras` = `main`, `crew` and `other`, the quad draws in the main view, in the crew-portrait face-cams (portraits on + occupied), and in an "Add Camera" secondary window respectively — and **never** in a part thumbnail in the VAB part list | ☐ | `CurrentPassBit` now classifies by `ViewportType` (`CharacterPortrait` → crew, `Secondary` → other, `PartThumbnail` → no bit); `RenderMainPass` still one overload / three call sites |
| 6 | **`/sim/display` still streams complete frames** (with opaque UI open — the 5348 item), and a secondary camera window open does not change what the stream shows | ☐ | `MainViewport.OffscreenTarget` is the shared `Program._offscreenTarget` attached via `IViewportLifecycle.AttachSharedTargets`; secondaries own their targets |
| 7 | **`always_render_iva` in the VAB** still forces interior meshes visible; `vessels/<id>/always_render` still keeps a distant vessel drawn | ☐ | `PartModel.AddInstance(…, IViewport, int)` gained a `RenderPartModels` early-out that the postfix mirrors; `Vehicle.UpdateRenderData(IViewport,int)` retyped |
| 8 | **Fallback gatOS menu (ModMenu absent):** the top-level "gatOS" menu opens and, with auto-hide on, the menu bar stays visible while it is open | ☐ | `MenuBarInUse` is written through `IGameViewportLifecycle.SetMenuBarInUse(true)` now |
| 9 | **plumetrail:** `ls /sim/debug/plumetrail/render/` lists ten fields and **no** `trail_color`; `cat json` has no `render/trail_color` key; the FX-editors card's item 5 (`erosion_max_depth`) works | ☐ | **C2** — the global tint is gone from the engine; trail colour is per template asset |
| 10 | **Provoke a structural failure** (hard landing on an engine — `CrashTolerance="3e6"` on EngineA2…A6): `ls /sim/vessels/by-id` gains `<id>_N` fragment/debris rows with `controllable` = `0`, their `orbit/time_to_ap` stops updating, `status`' controlled vessel moves to the surviving controllable fragment, and any weld/sticker anchored on a shed part goes dormant rather than drawing garbage | ☐ | the new part-failure/debris system; no `debris` flag on the surface yet (follow-up) |
| 11 | **Parachutes:** on a `ParachuteBayB` vessel, `ctl/stage` in the chute's sequence arms the chutes (bay doors open, `animations/<n>` reflects it) and the drogue/main deploy on descent; `acceleration_body` shows chute drag; `parts/<n>/display_name` reads the authored names ("Parachute Bay", "Drogue Radial A") | ☐ | new `ISequenced` modules; `Part.DisplayName` authored (not unique) |
| 12 | `cat /sim/status/accessors` is clean after a flight exercising throttle, translate/rotate, staging, an FC setpoint, a light toggle and an `/sim/audio` clip | ☐ | standing post-update check; every `Brutal.*.dll` was rebuilt this drop (declared surfaces identical) — FMOD/ImGui behaviour is a live check |
| 13 | **FX reflection health on first launch:** `cat /sim/debug/fx/health` (and `/sim/status/accessors`) resolves every handle — in particular `VolumetricExhaustRenderer._currentAtmosphericPressure` and `_debugThrottle`, and a `/sim/debug/engineplume` edit still propagates to a live plume | ☐ | decomp lag risk: revs 5349–5399 have no changelog and that renderer grew ~1248 lines; the two fields moved (`:278 → :290`, `:306 → :310`) in a field reshuffle, so the shipping DLL is the only proof the names survived |
| 14 | **Sticker decal actually draws** after the GLSL is recompiled against 5402: place one on a vessel and one on terrain, then toggle `paint/stickers/debug` — the magenta projection-box checker appears, occludes correctly and tracks the vessel | ☐ | the runtime-compiled sticker GLSL can hit a **stale SPIR-V cache**; the shared headers are untouched this drop so the risk is lower than at 5348, but only a live draw settles it. `GridPass` moved to per-`ShaderSlot` depth descriptor sets — gatOS binds none of it and keeps its own, written against `Program.OffscreenTarget.DepthImage` |
| 15 | **thug_life quad on a part thumbnail / crew portrait target:** with the quad enabled, open the VAB part list and a crew portrait — no validation error, no device loss, nothing drawn on the thumbnail | ☐ | those viewports now have distinct `ViewportOptionFlags` presets and the part-thumbnail target is built in `ViewportRegistry.CreatePartThumbnailViewport`; the pipeline stamp (`OffscreenTarget.SetupGraphicsPipeline`) is unchanged code but the format/sample handshake with those targets was not read statically |
...
# Custom clutter textures live KSA checklist (2026.8.19.5261; **texture-id scheme corrected at `2026.8.22.5348`**)
...
- [ ] `cat /sim/paint/textures/clutter` on a loaded flight lists real stock textures with plausible
  ...
  **This item could not have passed before the 5348 pass (C4, a PRE-EXISTING bug identical on 5261):**
  rows were keyed on `TextureReference.GetRealId()`, which returns `Id` only when
  `SerializedId.IsReferenceable`, and that is set only when the asset XML carries an `Id=` **attribute**
  — not one clutter texture element in `Content/Core/GroundClutter/{Grass,GenericRock,EarthTrees}Assets.xml`
  has one. Every slot fell through to `Anonymous++`, the catalog published **empty** and every `bind`
  answered `ENOENT`; the feature was completely inert. Rows are now keyed on
  `TextureReference.LocalPath`, so the `texture_id` values used throughout the rest of this checklist
  are **content-relative paths** (e.g. `Textures/Planets/Earth/GroundClutter/Grass_Diffuse.ktx2`), not
  the `EarthGrassClutterDiffuse`-style ids previously documented — those live in an inline
  `<Material Id="EarthGrassClutterMaterial">` block in `Astronomicals.xml` that is **never
  deserialized**, so that content layout does not exist in either build. Bindings are session-only, so
  there is nothing to migrate. The walk also covers the new `alpha` slot (C5); no stock clutter
  material authors one yet, so it should be absent from the listing.

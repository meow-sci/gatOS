...
<Aside type="tip" title="The same files, from your own machine">
  ...
  on your host machine could also fetch the same values with a plain HTTP request or MQTT client. AI agents can use
  the first-class MCP interface instead; it groups the same game model into logical JSON resources and tools rather
  than mirroring every filesystem leaf (see the [MCP API reference](/gatOS/reference/mcp/)).

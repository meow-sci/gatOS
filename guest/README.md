...
which supervises exactly five things: `init-gatos` (sysinit: mounts — including the unified
cgroup2 hierarchy at `/sys/fs/cgroup` with all controllers delegated, so container runtimes
(`apk add podman`) work out of the box (guest v17+; OpenRC's cgroups service would do this on
stock Alpine) — static slirp network 10.0.2.15/gw 10.0.2.2, device nodes), OpenSSH `sshd`
(`-D -e`, key-only root login via

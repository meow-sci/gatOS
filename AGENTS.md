...
made the same way, end to end, with nothing forgotten. `CLAUDE.md` states *that* the SPEC/scope
...
but still obey the G2 dependency rule and threading rules (`CLAUDE.md`).
...
| `CLAUDE.md` | feature lands | status-table row; threading-rules paragraph if a new game-thread work site or patch is added |
...
| `.claude/skills/gatos/` + `docs/TUTORIAL_DATA_REFERENCE.md` | change affects how programs are written | refresh recipes/reference |
...
- [ ] SPEC + matrix + scope pages + CLAUDE.md + VALIDATION + MILESTONES updated (§9).

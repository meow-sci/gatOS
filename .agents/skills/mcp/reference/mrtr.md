...
resource: "https://github.com/modelcontextprotocol/csharp-sdk/blob/main/docs/concepts/mrtr/mrtr.md"
...
Under `2026-07-28`, MRTR is the recommended way to obtain client input from a server handler. The spec removes the legacy server-to-client `elicitation/create`, `sampling/createMessage`, and `roots/list` request methods, so any code that needs to work on a `2026-07-28` Streamable HTTP server (where Streamable HTTP no longer supports sessions) must use `InputRequiredException` rather than `ModelContextProtocol.Server.McpServer.ElicitAsync`, `ModelContextProtocol.Server.McpServer.SampleAsync`, or `ModelContextProtocol.Server.McpServer.RequestRootsAsync`. The legacy methods still work on stateful sessions — including `2026-07-28` stdio sessions — but they throw `InvalidOperationException("X is not supported in stateless mode.")` on every stateless session.
...
A tool participates in MRTR by throwing `ModelContextProtocol.Protocol.InputRequiredException` with an `ModelContextProtocol.Protocol.InputRequiredResult` describing what it needs. On retry, the client's responses arrive on the request parameters and the tool inspects them to decide what to do next.

### Checking MRTR support

Tools should check `ModelContextProtocol.Server.McpServer.IsMrtrSupported` before throwing `InputRequiredException`. The property returns `true` when either:
...
Throw `ModelContextProtocol.Protocol.InputRequiredException` to return an incomplete result. The exception carries an `ModelContextProtocol.Protocol.InputRequiredResult` containing `inputRequests` and/or `requestState`:
...
- `ModelContextProtocol.Protocol.RequestParams.InputResponses` — a dictionary of client responses keyed by the same keys used in `inputRequests`.
- `ModelContextProtocol.Protocol.RequestParams.RequestState` — the opaque state string echoed back by the client.

Use `ModelContextProtocol.Protocol.InputResponse.Deserialize` with the `JsonTypeInfo<T>` matching the response type. The expected type follows from the matching `ModelContextProtocol.Protocol.InputRequest.Method` in the original `inputRequests` map — there is no on-the-wire discriminator.
...
When `ModelContextProtocol.Server.McpServer.IsMrtrSupported` is `false`, the tool
...
deserialized `ModelContextProtocol.Protocol.ElicitResult` carries an
`ModelContextProtocol.Protocol.ElicitResult.Action` of `"accept"`, `"decline"`, or
`"cancel"`, surfaced through the
`ModelContextProtocol.Protocol.ElicitResult.IsAccepted` shorthand.

...
resource: "https://github.com/modelcontextprotocol/csharp-sdk/blob/main/docs/concepts/resources/resources.md"
...
- Using the `ModelContextProtocol.Server.McpServerResourceAttribute` attribute on methods within a class marked with `ModelContextProtocol.Server.McpServerResourceTypeAttribute`
- Using `ModelContextProtocol.Server.McpServerResource.Create` factory methods from a delegate, `MethodInfo`, or `AIFunction`
- Deriving from `ModelContextProtocol.Server.McpServerResource` or `ModelContextProtocol.Server.DelegatingMcpServerResource`
...
Text resources return their content as `ModelContextProtocol.Protocol.TextResourceContents` with a `Text` property:
...
Binary resources return their content as `ModelContextProtocol.Protocol.BlobResourceContents` with a `Blob` property containing the raw bytes. Use the `ModelContextProtocol.Protocol.BlobResourceContents.FromBytes` factory method to construct instances:
...
Clients can discover and read resources using `ModelContextProtocol.Client.McpClient`:

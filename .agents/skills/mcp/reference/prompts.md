...
resource: "https://github.com/modelcontextprotocol/csharp-sdk/blob/main/docs/concepts/prompts/prompts.md"
...
- Using the `ModelContextProtocol.Server.McpServerPromptAttribute` attribute on methods within a class marked with `ModelContextProtocol.Server.McpServerPromptTypeAttribute`
- Using `ModelContextProtocol.Server.McpServerPrompt.Create` factory methods from a delegate, `MethodInfo`, or `AIFunction`
- Deriving from `ModelContextProtocol.Server.McpServerPrompt` or `ModelContextProtocol.Server.DelegatingMcpServerPrompt`
...
The attribute-based approach is the most common and is shown throughout this document. Prompts can return `ChatMessage` instances for simple text/image content, or `ModelContextProtocol.Protocol.PromptMessage` instances when protocol-specific content types like `ModelContextProtocol.Protocol.EmbeddedResourceBlock` are needed.
...
Prompt messages can contain more than just text. For text and image content, use [`ChatMessage`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.chatmessage) from Microsoft.Extensions.AI. `DataContent` is automatically mapped to the appropriate MCP content block: image MIME types become `ModelContextProtocol.Protocol.ImageContentBlock`, audio MIME types become `ModelContextProtocol.Protocol.AudioContentBlock`, and all other MIME types become `ModelContextProtocol.Protocol.EmbeddedResourceBlock` with binary resource contents. For text-embedded resources specifically, use `ModelContextProtocol.Protocol.PromptMessage` directly.
...
For protocol-specific content types like `ModelContextProtocol.Protocol.EmbeddedResourceBlock`, use `ModelContextProtocol.Protocol.PromptMessage` instead of `ChatMessage`. `PromptMessage` has a `Role` property and a single `Content` property of type `ModelContextProtocol.Protocol.ContentBlock`:
...
For binary resources, use the `ModelContextProtocol.Protocol.BlobResourceContents.FromBytes` factory method:
...
Clients can discover and use prompts through `ModelContextProtocol.Client.McpClient`.
...
Servers can dynamically add, remove, or modify prompts at runtime and notify connected clients. These are unsolicited notifications, so they require [stateful mode or stdio](stateless.md) — [stateless](stateless.md#stateless-mode-recommended) servers cannot send unsolicited notifications.

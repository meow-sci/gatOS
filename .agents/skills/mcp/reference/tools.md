...
resource: "https://github.com/modelcontextprotocol/csharp-sdk/blob/main/docs/concepts/tools/tools.md"
...
- Using the `ModelContextProtocol.Server.McpServerToolAttribute` attribute on methods within a class marked with `ModelContextProtocol.Server.McpServerToolTypeAttribute`
- Using `ModelContextProtocol.Server.McpServerTool.Create` factory methods from a delegate, `MethodInfo`, or `AIFunction`
- Deriving from `ModelContextProtocol.Server.McpServerTool` or `ModelContextProtocol.Server.DelegatingMcpServerTool`
- Implementing a custom `ModelContextProtocol.Server.McpRequestHandler`2` via `ModelContextProtocol.Server.McpServerHandlers`
- Implementing a low-level `ModelContextProtocol.Server.McpRequestFilter`2`

The attribute-based approach is the most common and is shown throughout this document. Parameters are automatically deserialized from JSON and documented using `[Description]` attributes. In addition to tool arguments, methods can accept special parameter types that are resolved automatically: `ModelContextProtocol.Server.McpServer`, `IProgress<ProgressNotificationValue>`, `ClaimsPrincipal`, and any service registered through dependency injection.
...
Tools can return various content types. The simplest is a `string`, which is automatically wrapped in a `ModelContextProtocol.Protocol.TextContentBlock`. For richer content, tools can return one or more `ModelContextProtocol.Protocol.ContentBlock` instances. Tools can also return [`DataContent`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.datacontent) from Microsoft.Extensions.AI, which is automatically mapped to the appropriate MCP content block: image MIME types become `ModelContextProtocol.Protocol.ImageContentBlock`, audio MIME types become `ModelContextProtocol.Protocol.AudioContentBlock`, and all other MIME types become `ModelContextProtocol.Protocol.EmbeddedResourceBlock` with binary resource contents.

#### Text content

Return a `string` or a `ModelContextProtocol.Protocol.TextContentBlock` directly:
...
Return an `ModelContextProtocol.Protocol.ImageContentBlock` with base64-encoded image data and a MIME type.
Use the `ModelContextProtocol.Protocol.ImageContentBlock.FromBytes` factory method or construct the block directly:
...
Return an `ModelContextProtocol.Protocol.AudioContentBlock` with base64-encoded audio data and a MIME type.
The `ModelContextProtocol.Protocol.AudioContentBlock.FromBytes` factory method encodes the raw bytes automatically:
...
Return an `ModelContextProtocol.Protocol.EmbeddedResourceBlock` to embed a resource directly in a tool result.
The resource can contain either text or binary data through `ModelContextProtocol.Protocol.TextResourceContents` or `ModelContextProtocol.Protocol.BlobResourceContents`:
...
For binary resources, use `ModelContextProtocol.Protocol.BlobResourceContents`:
...
Set `UseStructuredContent = true` on `ModelContextProtocol.Server.McpServerToolAttribute` or
`ModelContextProtocol.Server.McpServerToolCreateOptions` to advertise an output schema and
serialize the return value into `ModelContextProtocol.Protocol.CallToolResult.StructuredContent`.
...
Any content block can include `ModelContextProtocol.Protocol.Annotations` to provide hints about the intended audience and priority:
...
Clients can discover and call tools using `ModelContextProtocol.Client.McpClient`:
...
Tool errors in MCP are distinct from protocol errors. When a tool encounters an error during execution, the error is reported inside the `ModelContextProtocol.Protocol.CallToolResult` with `ModelContextProtocol.Protocol.CallToolResult.IsError` set to `true`, rather than as a protocol-level exception. This allows the LLM to see the error and potentially recover.
...
- `ModelContextProtocol.McpProtocolException` is re-thrown as a JSON-RPC error response (not a tool error result).
- `OperationCanceledException` is re-thrown when the cancellation token was triggered.

For all other exceptions, the error is returned as a tool result. If the exception derives from `ModelContextProtocol.McpException` (excluding `McpProtocolException`, which is re-thrown above), its message is included in the error text; otherwise, a generic message is returned to avoid leaking internal details.
...
Throw `ModelContextProtocol.McpProtocolException` to signal a protocol-level error (for example, invalid parameters or unknown tool). These exceptions propagate as JSON-RPC error responses rather than tool error results:
...
On the client side, inspect the `ModelContextProtocol.Protocol.CallToolResult.IsError` property after calling a tool:
...
Servers can dynamically add, remove, or modify tools at runtime. When the tool list changes, the server notifies connected clients so they can refresh their tool list. These are unsolicited notifications, so they require [stateful mode or stdio](stateless.md) — [stateless](stateless.md#stateless-mode-recommended) servers cannot send unsolicited notifications.

#### Sending notifications from the server

Inject `ModelContextProtocol.Server.McpServer` and call the notification method after modifying the tool list:
...
When using the Streamable HTTP transport, tool parameters can be mirrored as HTTP headers so that network infrastructure (load balancers, proxies, gateways) can make routing decisions without parsing the JSON-RPC request body. Apply the `ModelContextProtocol.Server.McpHeaderAttribute` to a parameter to opt it in:
...
By default, `Mcp-Param-*` headers are sent only for tools discovered via `ModelContextProtocol.Client.McpClient.ListToolsAsync`. If a client already has tool schema information (for example, from a previous session, hardcoded configuration, or an out-of-band source), it can pre-load those definitions so that headers are sent immediately—without a round trip to the server.
...
Known tools survive `ModelContextProtocol.Client.McpClient.ListToolsAsync` cache clears—they remain in the cache even when the server's tool list is refreshed. If the server returns a tool with the same name, the server's definition overwrites the cached one, but the tool keeps its known status.

To remove known tools, use `ModelContextProtocol.Client.McpClient.RemoveKnownTools` for specific tools or `ModelContextProtocol.Client.McpClient.ClearKnownTools` to remove all:
...
All tools passed to `ModelContextProtocol.Client.McpClient.AddKnownTools` are validated for correct `x-mcp-header` annotations. If any tool in the batch fails validation, an `System.ArgumentException` is thrown and no tools are added (all-or-nothing).

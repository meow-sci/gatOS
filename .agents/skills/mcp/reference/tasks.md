...
resource: "https://github.com/modelcontextprotocol/csharp-sdk/blob/main/docs/concepts/tasks/tasks.md"
...
with a `ModelContextProtocol.Extensions.Tasks.CreateTaskResult` instead of the standard result
(for example, `ModelContextProtocol.Protocol.CallToolResult`). The client then polls `tasks/get`
...
`ModelContextProtocol.Extensions.Tasks.McpTaskStatus` wire values are serialized in snake_case:
`working`, `input_required`, `completed`, `cancelled`, `failed`.

The discriminator field `ModelContextProtocol.Protocol.Result.ResultType?displayProperty=nameWithType`
on the response payload is `"task"` for `ModelContextProtocol.Extensions.Tasks.CreateTaskResult`
...
`ModelContextProtocol.Extensions.Tasks.McpTasksBuilderExtensions.WithTasks` on the server builder,
passing an `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore`.
The SDK ships `ModelContextProtocol.Extensions.Tasks.InMemoryMcpTaskStore` for development and tests:
...
- Advertises the `io.modelcontextprotocol/tasks` extension in
  `ModelContextProtocol.Protocol.ServerCapabilities.Extensions?displayProperty=nameWithType`.
...
- Establishes a task scope so that `ModelContextProtocol.Server.McpServer.ElicitAsync`,
  `ModelContextProtocol.Server.McpServer.SampleAsync`, and
  `ModelContextProtocol.Server.McpServer.RequestRootsAsync` called from inside the tool
...
TTL-based cleanup, implement `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore` yourself
...
`ModelContextProtocol.Server.McpServerHandlers.CallToolWithAlternateHandler?displayProperty=nameWithType`.
It returns a `ModelContextProtocol.Protocol.ResultOrAlternate`1`, so each invocation can choose
...
> `ModelContextProtocol.Server.McpServerHandlers.CallToolHandler?displayProperty=nameWithType`
> and `ModelContextProtocol.Server.McpServerHandlers.CallToolWithAlternateHandler?displayProperty=nameWithType`
...
`ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.CallToolWithPollingAsync`
...
- Dispatches input requests through the client's registered handlers
  (`ModelContextProtocol.Client.McpClientHandlers.SamplingHandler` and
...
- Deduplicates already-resolved input request keys across polls so each request is handled at
  most once.
- Returns the final `ModelContextProtocol.Protocol.CallToolResult` when the task completes,
  or throws `ModelContextProtocol.McpException` on `Failed`/`Cancelled`.
...
Use `ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.CallToolAsTaskAsync` to receive the raw
`ModelContextProtocol.Extensions.Tasks.ResultOrCreatedTask`1` without auto-polling, then drive the
lifecycle yourself using `ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.GetTaskAsync`,
`ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.UpdateTaskAsync`, and
`ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.CancelTaskAsync`:
...
`ModelContextProtocol.Extensions.Tasks.McpTaskStatus.InputRequired` across many consecutive polls
...
`ModelContextProtocol.McpException`. This guards against a server that never transitions
...
`ModelContextProtocol.Extensions.Tasks.McpTaskStatus.InputRequired` and returns the outstanding
requests in `ModelContextProtocol.Extensions.Tasks.InputRequiredTaskResult.InputRequests`. Each
...
`ModelContextProtocol.Extensions.Tasks.McpTasksClientExtensions.UpdateTaskAsync`, keyed by the same identifiers.

Supported input request methods:

| Method | Dispatched to the client handler |
| --- | --- |
| `elicitation/create` | `ModelContextProtocol.Client.McpClientHandlers.ElicitationHandler` |
| `sampling/createMessage` | `ModelContextProtocol.Client.McpClientHandlers.SamplingHandler` |
...
- Servers **should** ignore `inputResponses` entries whose key does not currently correspond to
  ...
  `ModelContextProtocol.Extensions.Tasks.InMemoryMcpTaskStore` follows this rule.

### Implementing a custom task store

Implement `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore` for production scenarios. Key
...
2. **Idempotent terminal transitions** —
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.SetCompletedAsync`,
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.SetFailedAsync`, and
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.SetCancelledAsync` must be no-ops on a task
...
3. **`InputResponseReceived` event** — after persisting an input response inside
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.ResolveInputRequestsAsync`, raise
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.InputResponseReceived?displayProperty=nameWithType`
...
4. **Strong-consistency on `CreateTaskAsync`** —
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.CreateTaskAsync` must not return until the
   task is durably persisted, so that a subsequent
   `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.GetTaskAsync` with the returned task ID
...
`ModelContextProtocol.Extensions.Tasks.McpTaskStatus.Completed` is the terminal status whenever the
underlying request produced its standard result, *including a
`ModelContextProtocol.Protocol.CallToolResult` with `IsError = true`*. Per SEP-2663,
tool-level error results are not promoted to `Failed`.

`ModelContextProtocol.Extensions.Tasks.McpTaskStatus.Failed` is reserved for JSON-RPC protocol-level
...
`ModelContextProtocol.Protocol.CallToolResult.IsError?displayProperty=nameWithType` for
...
`ModelContextProtocol.Extensions.Tasks.InMemoryMcpTaskStore` uses immutable record snapshots with
...
- **Lazy task creation**: when a tool runs through the task store, the store's
  `ModelContextProtocol.Extensions.Tasks.IMcpTaskStore.CreateTaskAsync` is invoked eagerly before
